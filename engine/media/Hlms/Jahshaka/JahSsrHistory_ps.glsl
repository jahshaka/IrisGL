...
// this one bounds what is STORED and the resolve's bounds what is CONSUMED, so
// a value that reaches the texture by any route other than this pass — or a
// reader that is wrong about what the texture holds — still cannot circulate.
// The FIRST frame after a workspace rebuild is not one of those cases: the
// chain clears this target black before anything reads it.)
//
...
void main()
{
	...
	// ORDERED, PER-CHANNEL COMPARISONS, AND NOT max(). GLSL and SPIR-V leave
	// `max` with a NaN operand UNDEFINED, and it lowers to a maxNum on the
	// platforms we ship (Metal/MoltenVK documents it, NVIDIA's FMNMX does it),
	// which returns the NON-NaN operand — so a single-channel NaN would have
	// read as finite and in range. lessThan/lessThanEqual are OpFOrd*: false
	// for a NaN in any channel, false for an Inf.
	//
	// A finite value too bright to be useful is held at the ceiling — clamp,
	// not min, so a channel below -kSsrMaxRadiance comes back too — and a
	...
	if( !all( lessThanEqual( abs( c.xyz ), vec3( kSsrMaxRadiance ) ) ) )
		c.xyz = all( lessThan( abs( c.xyz ), vec3( 3.0e38 ) ) )
					? clamp( c.xyz, vec3( -kSsrMaxRadiance ), vec3( kSsrMaxRadiance ) )
					: vec3( 0.0 );
	fragColour = c;
}

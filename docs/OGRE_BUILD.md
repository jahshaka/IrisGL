...
The engine is the pinned `thirdparty/ogre-next` submodule (upstream master, 4.0.0-unstable). A stock checkout
does NOT work: our changes ride as the numbered patch stack in `thirdparty/ogre-patches/`, and
`scripts/build-ogre.sh` is the whole build — it applies the stack idempotently, configures, builds
(RelWithDebInfo, shader-compile threading mode 2, an explicitly pinned Component set) and installs into THIS
tree's `thirdparty/ogre-next-install`. Every tree and worktree owns its install. Never configure the submodule
by hand.

When the stack changed, reset first, INSIDE the submodule (from the irisgl directory the same commands leave the
patched files behind and the script stops on an overlap false positive):
```bash
git -C thirdparty/ogre-next checkout -- . && git -C thirdparty/ogre-next clean -fd && ./scripts/build-ogre.sh
```
...
## Runtime data
The Hlms templates are required at run time. The Studio build stages Ogre's media from the PATCHED SOURCE tree
beside `engine/media`, so a MEDIA patch must be applied in the tree even though nothing recompiles.
